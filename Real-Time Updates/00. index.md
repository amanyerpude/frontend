...
- ## Short Polling  
	...
	- ### Short Polling. Scaling
	    ...
	    - #### There are two types of scaling.
	        - ###### Vertical Scaling or Scaling Up (add CPU, RAM etc) ![image](media/Vertical_Scaling_or_Scaling_Up_add_CPU_RAM_etc.jpg)
	        - ###### Horizontal Scaling or Scaling Out (add servers) ![image](media/Horizontal_Scaling_or_Scaling_Out_add_servers.jpg)
	    - #### Load Balancer ![image](media/Load_Balancer.jpg)
	    ...
	    - ####  What problems we may struggle ?
	        ...
	        - ##### What can we do if the number of clients has become too large ?
	            - ###### We can simply add more CPU and RAM to the server. ![image](media/add_more_CPU_and_RAM_to_the_server.jpg)
	        - ##### What if we reach the limit ?
	            - ###### Then we must do Horizontal Scaling. ![image](media/Horizontal_Scaling.jpg)
- ## Long Polling 
	- ![image](media/Long_Polling.jpg)
    ...
    - ### Long Polling. Problems
        - #### Long Polling. Delay
            - Why is this a problem ? ![image](media/Long_Polling_Problems.png)
    ...
    - #### Long Polling. Scaling
        ...
        - Before we get started, we need to understand the difference between Stateless and Stateful server architecture. ![image](media/Stateless_and_Stateful_server_ar.png)
        - Let’s take a look at what can happen if we start scaling Stateful servers. ![image](media/Scaling_Stateful_Servers.png)
        - ##### Solution #1. Sticky Session ![image](media/Sticky_Session.png)
        - ##### Solution #2. Shared Storage ![Shared_Storage.png](media/Shared_Storage.png)
...
- ## Server Sent Events
    - Server-Sent Events (SSE) is an implementation of **Comet** in which a client establishes a connection to a server, allowing the server to directly send messages to the client. Unlike Short/Long polling, SSE is not just a hack, but a push technology that allows the server to send messages to the client. ![Server_Sent_Events.png](media/Server_Sent_Events.png)
    - ### Pros:
    ...
    - ### Server Sent Events. When we should avoid it 
	    - ![Server_Sent_Events_When_we_shoul.png](media/Server_Sent_Events_When_we_shoul.png)
